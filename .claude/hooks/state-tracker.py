...
def parse_status_from_markdown(content: str) -> Dict[str, Any]:
    ...
    if coverage_match:
        info["coverage"] = (
            ...
            if "." in coverage_match.group(1)
            else float(coverage_match.group(1))
        )
...
def parse_tasks_from_markdown(content: str) -> Dict[str, Any]:
    ...
    task_blocks = re.split(r"###\s+(TK-\d+)", tasks_section)[
        1:
    ...
    for i in range(0, len(task_blocks), 2):
        ...
        completed_match = re.search(
            r"\*\*Completed:\*\*\s*(\d{4}-\d{2}-\d{2})", task_content
        )
...
def parse_entities_from_markdown(content: str) -> Set[str]:
    ...
    entity_matches = re.finditer(
        r"###\s+Entity:\s+(?:\[(\w+)\]|(\w+))", data_model_section
    )
...
def check_constitution_compliance() -> Dict[str, Any]:
    ...
    coverage_target = (
        quality_bars.get("coverage_target", 0.85)
        if isinstance(quality_bars, dict)
        else 0.85
    )
