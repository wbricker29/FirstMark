...
FIRST_NAMES = ["Sarah", "Marcus", "Elena", "David", "Priya", "James", "Maya", "Robert",
               "Aisha", "Michael", "Jennifer", "Alex", "Keisha", "Daniel", "Lisa"]
LAST_NAMES = ["Chen", "Johnson", "Rodriguez", "Kim", "Patel", "Williams", "Thompson",
              "Garcia", "Anderson", "Lee", "Martinez", "Brown", "Taylor", "Davis"]

# Real FirstMark portfolio companies (use some for realism)
CURRENT_COMPANIES = [
    "Stripe", "Plaid", "DataDog", "Ramp", "Astronomer", "Betterment",
    "Narmi", "Contentful", "TechCorp", "ScaleUp Inc", "CloudFlow", "DataWorks",
    "FinTech Solutions", "InfraCo", "DevTools Inc", "PaymentsCo"
]

PORTFOLIO_COMPANIES = [
    "AcmeCo", "DataFlow", "FinanceStream", "CloudScale", "PayNow"
]

LOCATIONS = ["New York", "San Francisco", "Austin", "Boston", "Seattle", "Chicago", "Remote"]

INDUSTRIES = ["FinTech", "Data Infrastructure", "SaaS", "Developer Tools", "Consumer", "B2B"]
...
def generate_guilds_csv(output_dir: Path, num_cto: int = 10, num_cfo: int = 8):
    ...
    with open(filepath, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=[
            ...
            'seniority', 'function', 'linkedin_url', 'guild'
        ])
        writer.writeheader()
        ...
        for i in range(num_cto):
            ...
            writer.writerow({
                ...
                'current_title': random.choice(['CTO', 'VP Engineering', 'Head of Engineering']),
        ...
        for i in range(num_cfo):
            ...
            writer.writerow({
                'name': name,
                'current_company': random.choice(CURRENT_COMPANIES),
                'current_title': random.choice(['CFO', 'VP Finance', 'Head of Finance']),
                'location': random.choice(LOCATIONS),
                'seniority': random.choice(['VP', 'C-Suite']),
                'function': 'Finance',
                'linkedin_url': f"https://linkedin.com/in/{name.lower().replace(' ', '')}",
...
def generate_exec_network_csv(output_dir: Path, num_execs: int = 12):
    ...
    with open(filepath, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=[
            'name', 'current_title', 'current_company', 'role_type',
            'location', 'linkedin_url', 'connection_source'
        ])
        writer.writeheader()
        ...
        for i in range(num_execs):
            ...
            writer.writerow({
                ...
                'current_company': random.choice(CURRENT_COMPANIES),
                'role_type': 'CTO' if 'CTO' in role_type or 'Engineering' in role_type else 'CFO',
                'location': random.choice(LOCATIONS),
                'linkedin_url': f"https://linkedin.com/in/{name.lower().replace(' ', '')}",
...
def generate_executive_bios_json(output_dir: Path, names: List[str]):
    ...
    prev_companies = ["Square", "Robinhood", "Coinbase", "Snowflake", "Databricks", "Airbnb"]
    ...
    for name in names[:15]:  # Generate bios for subset of executives
        ...
        if role_type == 'CTO':
            bio_text = template.format(
                ...
                focus=random.choice(['payment infrastructure', 'data platforms', 'developer tools', 'cloud infrastructure']),
                prev_role=random.choice(['Director of Engineering', 'Engineering Manager', 'Principal Engineer']),
                prev_company=random.choice(prev_companies),
                stage='Series B through IPO' if random.random() > 0.5 else 'Series A to Series C',
            ...
            bio_text = template.format(
                ...
                prev_role=random.choice(['VP Finance', 'Director of FP&A', 'Finance Manager']),
                prev_company=random.choice(prev_companies),
                phase=random.choice(['hypergrowth', 'Series B and C fundraising', 'the IPO process']),
                education=random.choice([
        ...
        bios.append({
            'name': name,
            'bio': bio_text,
            'source': 'LinkedIn About + Press' if random.random() > 0.5 else 'Company bio + LinkedIn'
        })
...
def main():
    ...
    all_names = [f"{random.choice(FIRST_NAMES)} {random.choice(LAST_NAMES)}"
