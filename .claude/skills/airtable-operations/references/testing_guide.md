...
⚠️  WARNINGS:
  • People → Normalized Function: Missing options [CPO, CRO, COO]
...
```bash
python scripts/load_candidates.py ../../../reference/guildmember_scrape.csv --dry-run --verbose
```
...
```bash
python scripts/validate_data.py \
  --csv ../../../reference/guildmember_scrape.csv \
...
- [ ] Normalized Function has all options (CFO, CTO, CPO, CRO, COO, CMO, CEO, Other)
...
- [ ] Normalized Function values are valid
...
# Check file exists
ls -la ../../../reference/guildmember_scrape.csv

# Verify CSV encoding (should be UTF-8)
file ../../../reference/guildmember_scrape.csv
```
