...
**Error:**
```
❌ ERROR: CSV file not found: reference/guildmember_scrape.csv
```
...
cd /path/to/project
python .claude/skills/airtable-file-loader/scripts/load_candidates.py reference/guildmember_scrape.csv
```
...
**Warning (non-fatal):**
```
⚠️ Skipping Normalized Function 'CPO' for Alex Rivera (not in schema)
```
...
- Non-CFO/CTO candidates will have blank Normalized Function
...
2. Click Normalized Function column header
3. Select "Edit field"
...
```bash
python load_candidates.py reference/guildmember_scrape.csv --dry-run --verbose
...
```bash
head -5 reference/guildmember_scrape.csv
