...
Examples:
    ...
    python validate_data.py --csv ../../../reference/guildmember_scrape.csv
...
IMPORTANT_FIELDS = ["Title", "Company", "LinkedIn URL", "Normalized Function"]
...
def validate_people_table(
    ...
    try:
        ...
        # records = mcp__airtable__list_records(baseId=BASE_ID, tableId=PEOPLE_TABLE_ID)

        # Mock data for demonstration
        records = []  # Would be populated by MCP call

        if not records:
            print("\n⚠️  VALIDATION FRAMEWORK READY")
            print("To use, replace mock MCP calls with actual MCP tool calls:")
            print("  • mcp__airtable__list_records(baseId, tableId)")
            return report
        ...
        for record in records:
            ...
            # Validate Normalized Function
            function = fields.get("Normalized Function", "")
