...
| Normalized Title | function, role_type, seniority, exec_function, normalized_function |
...
Automatically infers `Normalized Title` from job titles:
...
if candidate.get('function') in VALID_FUNCTIONS:
    ...
    print(f"⚠️  Skipping Normalized Title '{candidate['function']}' (not in schema)")
```
...
```bash
# Test with dry-run
./scripts/quick_load.sh  --dry-run

# Load candidates
./scripts/quick_load.sh 
