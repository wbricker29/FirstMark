...
# Import Airtable utilities (using local module)
from airtable_utils import create_record, list_records, PEOPLE_TABLE_ID, DEFAULT_BASE_ID

# Configuration imported from airtable_utils
...
def infer_function(title: str) -> str:
    """Infer Normalized Title from title (returns full Airtable option names)."""
    title_upper = title.upper()

    # Order matters - check most specific first
    # Return full option names as they appear in Airtable schema
...
def check_duplicates(
    ...
    """
    try:
        # Fetch existing people from Airtable
        existing_people = list_records(
            base_id=BASE_ID,
            table_id=PEOPLE_TABLE_ID,
            max_records=500,  # Adjust if more than 500 people expected
        )
...
def create_airtable_records(
    ...
    # Known valid options (from demo schema - must match Airtable single-select options exactly)
    VALID_FUNCTIONS = {
    ...
    for idx, candidate in enumerate(candidates, start=1):
        try:
            # Build fields dict - only required fields (Created timestamp auto-generated by Airtable)
            ...
            # NOTE: Current Company is a linked field (multipleRecordLinks to Companies table)
            # Cannot set directly with company name string - must use company record IDs
            # Use scripts/fix_company_links.py after import to link companies
            if candidate.get("company") and verbose:
                print(
                    f"   ⚠️  Skipping Current Company for {candidate['name']} (linked field - use fix_company_links.py after import)"
                )
            ...
            if candidate.get("function") in VALID_FUNCTIONS:
                fields["Normalized Title"] = candidate["function"]
            elif verbose and candidate.get("function"):
                print(
                    f"   ⚠️  Skipping Normalized Title '{candidate['function']}' for {candidate['name']} (not in schema)"
                )
            ...
            result = create_record(
                base_id=BASE_ID, table_id=PEOPLE_TABLE_ID, fields=fields
            )
...
Examples:
  ...
  %(prog)s ../Exec_Network.csv --verbose
    ...
    if not args.dry_run and created_count > 0:
        ...
        print("\n⚠️  IMPORTANT: Current Company field not populated (linked field)")
        print("   Run: python scripts/fix_company_links.py")
        print("   This will create company records and link them to people.")
    elif args.dry_run:
