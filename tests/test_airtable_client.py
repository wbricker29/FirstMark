...
def mock_tables(mock_api):
    ...
    tables = {
        ...
        "Operations-Automation_Log": MagicMock(),
    }
...
def client(mock_api, mock_tables):
    ...
    with patch("demo.airtable_client.Api", return_value=mock_api):
        ...
        client_instance.automation_log = mock_tables["Operations-Automation_Log"]
...
def test_init_with_valid_credentials(mock_api):
    ...
    assert (
        mock_api.table.call_count == 3
    )  # 3 write-only tables (screens, assessments, automation_log)
...
# ============================================================================
# Tests for get_screen method (DEPRECATED - method removed in simplification)
# ============================================================================


@pytest.mark.skip(
    reason="get_screen() method removed - use ScreenWebhookPayload instead"
)
...
def test_get_screen_with_master_screen_spec_formula(client, mock_tables):
    """Test get_screen when Master Screen Spec formula field is populated."""
    # Mock screen record with Master Screen Spec formula populated
    mock_tables["Platform-Screens"].get.return_value = {
        "id": "recScreen123",
        "fields": {
            "status": "Ready to Screen",
            "Search": ["recSearch456"],
            "Spec Selection Mode": "Custom Spec",
            "Custom Spec Text": "# Custom CFO Criteria\n\nAd-hoc evaluation...",
            "Master Role Spec Content": "# Custom CFO Criteria\n\nAd-hoc evaluation...",  # Formula output
            "Candidates": ["recCandidate1"],
        },
    }
    ...
    mock_tables["Platform-Searches"].get.return_value = {
        "id": "recSearch456",
        "fields": {"Role Spec": ["recRoleSpec789"]},
    }

    # Mock candidate
    mock_tables["People"].get.return_value = {
        "id": "recCandidate1",
    ...
    # Verify Master Screen Spec formula value is returned
    assert (
        result["role_spec_markdown"] == "# Custom CFO Criteria\n\nAd-hoc evaluation..."
    )
    # role_spec_id should still be available for backward compatibility
    assert result["role_spec_id"] == "recRoleSpec789"
...
def test_get_screen_with_search_spec_mode(client, mock_tables):
    """Test get_screen when using Search Spec mode (default)."""
    # Mock screen record with Search Spec mode (Master Screen Spec pulls from linked spec)
    mock_tables["Platform-Screens"].get.return_value = {
        "id": "recScreen123",
        "fields": {
            "status": "Ready to Screen",
            "Search": ["recSearch456"],
            "Spec Selection Mode": "Search Spec",
            # Master Role Spec Content formula would reference Role_Specs.Spec Content via Search link
            # For testing, we simulate the formula returning the linked spec content
            "Master Role Spec Content": "# CFO - Series B SaaS\n\nFrom linked spec...",
            "Candidates": ["recCandidate1"],
        },
    }
    ...
    # Verify formula returns linked spec content
    assert (
        result["role_spec_markdown"] == "# CFO - Series B SaaS\n\nFrom linked spec..."
    )
    assert result["role_spec_id"] == "recRoleSpec789"
...
def test_get_screen_backward_compatibility_no_formula(client, mock_tables):
    """Test get_screen falls back to role_spec_id when Master Screen Spec formula absent."""
    # Mock screen record WITHOUT Master Screen Spec formula (old schema)
    mock_tables["Platform-Screens"].get.return_value = {
        "id": "recScreen123",
        "fields": {
            "status": "Ready to Screen",
            "Search": ["recSearch456"],
            # No "Master Screen Spec" field
            "Candidates": ["recCandidate1"],
        },
    }
    ...
    # Verify backward compatibility: role_spec_id is populated, markdown is None
    assert result["role_spec_id"] == "recRoleSpec789"
    assert result["role_spec_markdown"] is None


# ============================================================================
# Tests for get_role_spec method (DEPRECATED - method removed in simplification)
# ============================================================================


@pytest.mark.skip(
    reason="get_role_spec() method removed - use ScreenWebhookPayload instead"
)
def test_get_role_spec_with_valid_spec_id(client, mock_tables):
    """Test successful role spec fetch with Spec Content field."""
    mock_tables["Platform-Role_Specs"].get.return_value = {
        "id": "recRoleSpec789",
        "fields": {
            "role_title": "CFO",
            "Spec Content": "# CFO Role Spec\n\n## Must-Haves...",
        },
    }
...
def test_get_role_spec_with_missing_markdown_field(client, mock_tables):
    ...
    with pytest.raises(RuntimeError, match="missing Spec Content field"):
...
def test_write_assessment_with_research_and_assessment(client, mock_tables):
    ...
    assert "Research JSON" in fields
    assert fields["Research Model"] == "o4-mini-deep-research"
    assert fields["Research Markdown Report"] == research.research_markdown_raw
    assert "Assessment Markdown Report" not in fields
...
def test_write_assessment_without_research(client, mock_tables):
    ...
    assert "Research Structured JSON" not in fields
    assert "Research Model" not in fields
    assert "Research Markdown Raw" not in fields
    assert "Assessment Markdown Report" not in fields
...
def test_write_assessment_without_role_spec(client, mock_tables):
    ...
    # Verify role_spec_markdown is not included when not provided
    call_args = mock_tables["Platform-Assessments"].create.call_args
    fields = call_args[0][0]

    assert "Role Spec Markdown Used" not in fields


def test_write_assessment_with_role_spec_markdown(client, mock_tables):
    """Test assessment write with role_spec_markdown parameter (stored in Assessment JSON)."""
    assessment = AssessmentResult(
        overall_score=85.0,
        overall_confidence="High",
        dimension_scores=[],
        ...
        assessment_timestamp=datetime(2025, 11, 17, 10, 0, 0),
        assessment_model="gpt-5-mini",
        role_spec_used="# Custom CFO Spec\n\nAd-hoc criteria...",
    )
    ...
    client.write_assessment(
        ...
        candidate_id="recCandidate456",
        assessment=assessment,
        role_spec_markdown="# Custom CFO Spec\n\nAd-hoc criteria...",
    )

    # Verify role_spec_markdown is NOT written as separate field (field doesn't exist)
    # Instead, it's stored in Assessment JSON as assessment.role_spec_used
    call_args = mock_tables["Platform-Assessments"].create.call_args
    fields = call_args[0][0]

    assert "Role Spec Markdown Used" not in fields
    # Verify role_spec_used is in the Assessment JSON
    import json

    assessment_json = json.loads(fields["Assessment JSON"])
    assert (
        assessment_json["role_spec_used"] == "# Custom CFO Spec\n\nAd-hoc criteria..."
    )


def test_write_assessment_with_markdown_report(client, mock_tables):
    """Test assessment write with markdown report fields."""
    assessment = AssessmentResult(
        overall_score=88.0,
        overall_confidence="High",
        dimension_scores=[],
        summary="Strong candidate with markdown report",
        assessment_timestamp=datetime(2025, 11, 17, 10, 0, 0),
        assessment_model="gpt-5-mini",
    )

    research = ExecutiveResearchResult(
        exec_name="Test Candidate",
        current_role="CFO",
        current_company="TestCo",
        research_summary="Test research",
        research_markdown_raw="# Test Research",
        research_timestamp=datetime(2025, 11, 17, 9, 0, 0),
        research_model="o4-mini-deep-research",
    )

    mock_tables["Platform-Assessments"].create.return_value = {
        "id": "recAssessmentMarkdown",
        "fields": {},
    }
    ...
    result = client.write_assessment(
        ...
        assessment=assessment,
        research=research,
        assessment_markdown="### Test Candidate\n**Overall Score:** 88.0/100",
    )

    assert result == "recAssessmentMarkdown"

    # Verify create was called with markdown fields
    call_args = mock_tables["Platform-Assessments"].create.call_args
    fields = call_args[0][0]

    assert (
        fields["Assessment Markdown Report"]
        == "### Test Candidate\n**Overall Score:** 88.0/100"
    )
    assert fields["Research Markdown Report"] == "# Test Research"
