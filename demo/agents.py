...
def merge_research_results(
    ...
    # Merge markdown content (inlined from _merge_markdown_content)
    markdown_blocks = (
        [merged.research_markdown_raw.strip()]
        if merged.research_markdown_raw and merged.research_markdown_raw.strip()
        else []
    )
    supplemental_markdown = (
        supplemental.research_markdown_raw or supplemental.research_summary
    )
    if supplemental_markdown and supplemental_markdown.strip():
        markdown_blocks.append(supplemental_markdown.strip())
...
def assess_candidate(
    ...
    if agent.model is not None:
        assessment.assessment_model = agent.model.id
