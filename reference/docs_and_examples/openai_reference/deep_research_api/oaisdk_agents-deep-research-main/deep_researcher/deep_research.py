...
from .agents.proofreader_agent import (
    ReportDraftSection,
    ReportDraft,
...
class DeepResearcher:
    ...
    async def run(self, query: str) -> str:
        ...
        final_report: str = await self._create_final_report(
            query, report_plan, research_results
        )

        elapsed_time = time.time() - start_time
        self._log_message(
            f"DeepResearcher completed in {int(elapsed_time // 60)} minutes and {int(elapsed_time % 60)} seconds"
        )
    ...
    async def _build_report_plan(self, query: str) -> ReportPlan:
        ...
        if self.verbose:
            ...
            self._log_message(
                f"Report plan created with {num_sections} sections:\n{message_log}"
            )
    ...
    async def _run_research_loops(self, report_plan: ReportPlan) -> List[str]:
        ...
        research_results = await asyncio.gather(
            *(
                run_research_for_section(section)
                for section in report_plan.report_outline
            )
        )
    ...
    async def _create_final_report(
        ...
        if use_long_writer:
            ...
            final_report = await ResearchRunner.run(self.proofreader_agent, user_prompt)
