...
2. **User Memory Agent** (`agent_with_user_memory.py`)
...
3. **Study Friend Agent** (`study_friend.py`)
...
4. **Image Agent** (`image_generation_model.py` & `image_generation_model.py`)
...
1. Create a new app at [Meta Apps Dashboard](https://developers.facebook.com/apps/)
1. Under "Use Case", select "Other"
...
2: **Set Up a Meta Business Account**
 ...
 3. **Setup Webhook**
   ...
   1. Click on "Configure a webhook".
   ...
   3. Run your app locally with `python <my-app>.py` and click "Verify and save".
