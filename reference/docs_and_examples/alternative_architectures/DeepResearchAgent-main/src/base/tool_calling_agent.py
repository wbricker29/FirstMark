...
from src.models import Model, agglomerate_stream_deltas, parse_json_if_needed
...
class ToolCallingAgent(MultiStepAgent):
    ...
    ) -> Generator[ChatMessageStreamDelta | ToolOutput]:
        """
        ...
        if parallel_calls:
            if len(parallel_calls) == 1:
                ...
                with ThreadPoolExecutor(self.max_tool_threads) as executor:
                    futures = [
                        executor.submit(process_single_tool_call, call_info)
                        for call_info in parallel_calls
                    ]
