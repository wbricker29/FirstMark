...
from src.base.multistep_agent import ActionOutput, ToolOutput, RunResult
...
class AsyncMultiStepAgent(ABC):
    ...
    def __init__(
            ...
            for key, value in EMPTY_PROMPT_TEMPLATES.items():
                if isinstance(value, dict):
                    for subkey in value.keys():
                        assert key in prompt_templates.keys() and (
                            subkey in prompt_templates[key].keys()
                        ), (
        ...
        self.final_answer_checks = (
            final_answer_checks if final_answer_checks is not None else []
        )
    ...
    def _setup_tools(self, tools, add_base_tools):
        assert all(isinstance(tool, AsyncTool) for tool in tools), (
            "All elements must be instance of AsyncTool (or a subclass)"
        )
            ...
            self.tools.update(
                {
                    ...
                    if name != "python_interpreter_tool"
...
{str(additional_args)}."""
        ...
        steps = [
            step
            async for step in self._run_stream(
                task=self.task, max_steps=max_steps, images=images
            )
        ]
        ...
        if self.return_full_result:
            ...
            if self.memory.steps and isinstance(
                getattr(self.memory.steps[-1], "error", None), AgentMaxStepsError
            ):
    ...
    async def _run_stream(
        ...
        while not returned_final_answer and self.step_number <= max_steps:
            ...
            if self.planning_interval is not None and (
                ...
                )  # Last yielded element should be a PlanningStep
            ...
            try:
                async for output in self._step_stream(action_step):
                    ...
                    if (
                        isinstance(output, (ActionOutput, ToolOutput))
                        and output.is_final_answer
                    ):
    ...
    async def _handle_max_steps_reached(
        self, task: str, images: list["PIL.Image.Image"]
    ) -> Any:
        action_step_start_time = time.time()
    ...
    async def _generate_planning_step(
        ...
        if is_first_step:
            ...
            if self.stream_outputs and hasattr(self.model, "generate_stream"):
                ...
                plan_message = self.model.generate(
                    input_messages, stop_sequences=["<end_plan>"]
                )
            ...
            input_messages = (
                [plan_update_pre] + memory_messages[:-1] + [plan_update_post]
            )
            if self.stream_outputs and hasattr(self.model, "generate_stream"):
                plan_message_content = ""
                input_tokens, output_tokens = 0, 0
        ...
        yield PlanningStep(
            ...
            model_output_message=ChatMessage(
                role=MessageRole.ASSISTANT, content=plan_message_content
            ),
            token_usage=TokenUsage(
    ...
    ) -> AsyncGenerator[ChatMessageStreamDelta | ActionOutput | ToolOutput, None]:
        """
    ...
    async def provide_final_answer(
        ...
        try:
            ...
            return ChatMessage(
                ...
                content=f"Error in generating final LLM output:\n{e}",
            )
