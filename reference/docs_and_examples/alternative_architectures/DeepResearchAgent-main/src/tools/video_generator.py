...
class OptimizedPromptTool(AsyncTool):
    ...
    async def forward(
        self, prompt: str, optimized_prompt: str, image_path: Optional[str] = None
    ) -> str:
...
class VideoGeneratorTool(AsyncTool):
    ...
    description: str = (
        "This tool generates a video based on a provided prompt and an image reference."
    )
    ...
    async def forward(
        ...
        if not optimized_prompt:
            logger.warning(
                "Failed to generate an optimized prompt, using the original prompt."
            )
    ...
    async def _generate_optimized_prompt(
        ...
        try:
            ...
            tools = [OptimizedPromptTool()]

            response = await self.analyzer_model(
                messages=messages, tools_to_call_from=tools
            )
            ...
            logger.info(
                f"VideoGeneratorTool generated optimized query: {optimized_prompt}"
            )
    ...
    async def _generate_video(
        self, prompt: str, save_name: str = "generated_video.mp4", image_path=None
    ) -> ToolResult:
        ...
        try:
            ...
            while video_data is None:
                try:
                    ...
                    logger.warning(
                        f"Failed to fetch video data: {e}, retrying in 60 seconds..."
                    )
