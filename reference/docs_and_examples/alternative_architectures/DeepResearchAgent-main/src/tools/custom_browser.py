...
class ExtractContentTool(AsyncTool):
    name: str = "extract_content"
    description: str = "Extract specific information from a webpage based on a goal."
    parameters: dict = {
        "type": "object",
        "properties": {
            "extracted_content": {
    ...
    async def forward(self, extracted_content) -> Any:
        ...
        Args:
            extracted_content: The content extracted from the page according to the goal

        Returns:
            ToolResult with the extracted content
        """
        return extracted_content


class BrowserUseTool(AsyncTool, Generic[Context]):
    ...
    async def _ensure_browser_initialized(self) -> BrowserContext:
        ...
        if self.browser is None:
            ...
            if self.browser_config:
                ...
                if self.browser_config.use_proxy:
        ...
        if self.context is None:
            ...
            if (
                self.browser_config
                and hasattr(self.browser_config, "new_context_config")
                and self.browser_config.new_context_config
            ):
                context_config = self.browser_config.new_context_config
        ...
        async with self.lock:
            try:
                ...
                max_content_length = getattr(
                    self.browser_config, "max_content_length", 5000
                )
                ...
                # Content extraction actions
                    ...
                    fetch_ = await fetch_url(url, self.converter)
                    content = fetch_.text_content

                    logger.info(f"Extracting content from page {url}: {content}")
