...
class OptimizedPromptTool(AsyncTool):
    ...
    description: str = (
        """Generates an prompt for image generation based on a given query."""
    )
...
class ImageGeneratorTool(AsyncTool):
    ...
    async def _generate_optimized_prompt(self, prompt: str) -> str:
        try:
            ...
            logger.info(
                f"ImageGeneratorTool generated optimized query: {optimized_prompt}"
            )
    ...
    async def _generate_image(
        self, prompt: str, save_name: str = "generated_image.png"
    ) -> ToolResult:
