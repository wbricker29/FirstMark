...
* `action`: The action must be one of the following: download, scroll_down, scroll_up, jump or search. The `action` is required.
 - download: Download the PDF from the given URL and open it in the browser. `pdf_url` and `save_name` are required.
 - scroll_down: Scroll down the PDF by one page. `pixels` is optional. If `pixels` is specified, scroll down by that many pixels.
 - scroll_up: Scroll up the PDF by one page. `pixels` is optional. If `pixels` is specified, scroll up by that many pixels.
 - jump: Jump to a specific page in the PDF. `page_number` is required. 
 - search: Search for a specific text in the PDF. `search_text` is required and must be a word or phrase. Will scroll to the first occurrence of the text.
* When you encounter the "Failed to load PDF document" error. You should find the pdf url (e.g., https://xxx/sample.pdf), and then download it and open it in browser.
 """,
        ...
        async def pdf_viewer(params: PDFAction, browser: BrowserContext):
            ...
            if action == "download":
                ...
                if not matches:
                    ...
                    msg = f"📄  Found {count} matches for the search text '{search_text}'. Scrolled to the first match."
                    logger.info(msg)
                    return ActionResult(extracted_content=msg, include_in_memory=True)
