...
async def build_agent(
    ...
    if default_mcp_tools is None:
        ...
        logger.info(
            f"| MCP Tools initialized: {', '.join([tool.name for tool in mcp_tools])}"
        )

    # Load Managed Agents
    if default_managed_agents is None:
        logger.info(
            "| No managed agents provided. Skipping managed agent initialization."
        )
    else:
        ...
        for managed_agent in default_managed_agents:
            if managed_agent.name not in used_managed_agents:
                logger.warning(
                    f"Managed agent '{managed_agent.name}' is not registered. Skipping."
                )
            else:
                managed_agent_tool = make_tool_instance(managed_agent)
                managed_agent_tools.append(managed_agent_tool)
        logger.info(
            f"| Managed agents initialized: {', '.join([agent.name for agent in managed_agent_tools])}"
        )
    ...
    if config.use_hierarchical_agent:
        ...
        for agent_name in used_managed_agents:
            if agent_name not in AGENT:
                logger.warning(
                    f"Managed agent '{agent_name}' is not registered. Skipping."
                )
            else:
        ...
        logger.info(
            f"| Managed agents initialized: {', '.join([agent.name for agent in managed_agents])}"
        )
        ...
        # Build agent
        agent = await build_agent(
            config, agent_config, default_tools=TOOL, default_mcp_tools=mcpadapt_tools
        )
