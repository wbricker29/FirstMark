...
from src.base import ToolOutput, StreamEvent

from src.memory import ActionStep, ToolCall, AgentMemory
from src.logger import LogLevel, YELLOW_HEX
...
class GeneralAgent(AsyncMultiStepAgent):
    ...
    ) -> AsyncGenerator[ChatMessageStreamDelta | ToolOutput]:
        """
    ...
    async def process_tool_calls(
        ...
        for tool_call in chat_message.tool_calls:
            ...
            model_outputs.append(
                str(f"Called Tool: '{tool_name}' with arguments: {tool_arguments}")
            )
            tool_calls.append(
                ToolCall(name=tool_name, arguments=tool_arguments, id=tool_call.id)
            )
        ...
        if parallel_calls:
            if len(parallel_calls) == 1:
                ...
                tasks = [
                    process_single_tool_call(call_info) for call_info in parallel_calls
                ]
        ...
        if final_answer_call:
            ...
            if isinstance(answer, str) and answer in self.state.keys():
                ...
                final_answer = await self.execute_tool_call(
                    "final_answer_tool", tool_arguments
                )
                self.logger.log(
