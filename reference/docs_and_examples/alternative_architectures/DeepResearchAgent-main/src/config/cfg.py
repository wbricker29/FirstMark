...
def process_general(config: MMConfig) -> MMConfig:
    ...
    config.log_path = os.path.join(
        config.exp_path, getattr(config, "log_path", "dra.log")
    )
    logger.info(f"| Arguments Log file: {config.log_path}")

    if "save_path" in config:
        config.save_path = os.path.join(
            config.exp_path, getattr(config, "save_path", "dra.json")
        )
...
def process_mcp(config: MMConfig) -> MMConfig:
    if "mcp_tools_config" in config:
        ...
        if "LocalMCP" in mcp_servers:
            ...
            args = [
                assemble_project_path(item) if item.endswith(".py") else item
                for item in args
            ]
