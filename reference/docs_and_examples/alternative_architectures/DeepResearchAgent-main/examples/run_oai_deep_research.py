...
async def answer_single_question(config, example):
    try:
        ...
        if example["file_name"]:
            ...
            if file_text.error:
                logger.warning(
                    f"Error reading file {example['file_name']}: {file_text.error}"
                )
...
def parse_args():
    ...
    parser.add_argument(
        "--config",
        default=os.path.join(root, "configs", "config_oai_deep_research.py"),
...
async def main():
    ...
    logger.info(
        "| Registed models: %s", ", ".join(model_manager.registed_models.keys())
    )
    ...
    tasks_to_run = [
        task for task in tasks_to_run[:-1]
    ]  # Remove the last task which is a test example
    logger.info(f"| Loaded {len(tasks_to_run)} tasks to run.")

    await answer_single_question(
        config,
        [
            task
            for task in tasks_to_run
            if task["task_id"] == "16cf70d8-9263-4eb0-a8a9-5eb91a23b462"
        ][0],
    )  # Run test example first
    exit()
