...
def parse_args():
    ...
    parser.add_argument(
        "--config",
        default=os.path.join(root, "configs", "config_general.py"),
...
if __name__ == "__main__":
    ...
    messages = [
        ChatMessage(
            role="user",
            content="Riddle solution: 200 coins, 30 face-up, must divide into two piles with equal face-down coins, unable to distinguish coin sides in darkness. Picks 30 coins, flips all, rest 170 untouched. Larger pile has 14 face-down coins. What is the outcome for the adventurer? Number of coins won or if he died.",
        ),
    ]

    response = asyncio.run(
        model_manager.registed_models["o3-deep-research"](
            messages=messages,
        )
    )
    print(response)
    ...
    response = asyncio.run(
        model_manager.registed_models["deepseek-chat"](
            messages=messages,
        )
    )
    print(response)
    ...
    response = asyncio.run(
        model_manager.registed_models["deepseek-reasoner"](
            messages=messages,
        )
    )
    print(response)
    ...
    response = asyncio.run(
        model_manager.registed_models["o3"](
            messages=messages,
        )
    )
    print(response)
    ...
    response = asyncio.run(
        model_manager.registed_models["claude37-sonnet"](
            messages=messages,
        )
    )
    print(response)
    ...
    response = asyncio.run(
        model_manager.registed_models["claude-4-sonnet"](
            messages=messages,
        )
    )
    print(response)
    ...
    response = asyncio.run(
        model_manager.registed_models["gemini-2.5-pro"](
            messages=messages,
        )
    )
    print(response)
